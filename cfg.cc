                ...
                if (sec.contains(jmptab_addr)) {
                    ...
                    while (1) {
                        ...
                        if (conflict_edge && (conflict_edge->jmptab <= jmptab_addr)) {
                            verbose(3,
                                    "removing switch edge 0x%016jx -> 0x%016jx (detected overlapping jump table or case)",
                ...
                if (sec.contains(jmptab_addr)) {
                    ...
                    while (1) {
                        ...
                        if (!target_sec->contains(case_addr)) {
                            ...
                            if (!conflict_edge) {
                                verbose(3, "adding switch edge 0x%016jx -> 0x%016jx", bb->insns.back().start,
                                        case_addr);
                                link_bbs(Edge::EDGE_TYPE_JMP_INDIRECT, bb, case_addr, jmptab_addr);
                            }
                        }
...
CFG::make_cfg(Binary *bin, std::list<DisasmSection> *disasm) {
    ...
    if (options.binary.type == Binary::BIN_TYPE_RAW) {
        ...
        disasm->erase(disasm->begin(), it);
        disasm->erase(std::next(it), disasm->end());
    }
