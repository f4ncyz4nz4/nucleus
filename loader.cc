...
#define APPLY_RELOC(x)          \
  ((x & ~bfd_howto->dst_mask) | \
   (((x & bfd_howto->src_mask) + relocation) & bfd_howto->dst_mask))
            ...
            bfd_vma data_offset = bfd_reloc->address * bfd_octets_per_byte(bfd_h, NULL);
...
int load_binary_dmp(std::string &fname, Binary *bin, Binary::BinaryType type) {
    int ret;
    long fsize;
    FILE *f;
    Section *sec;
...
int load_binary(std::string &fname, Binary *bin, Binary::BinaryType type) {
    ...
    } else if (type == Binary::BIN_TYPE_DMP) {
        return load_binary_dmp(fname, bin, type);
    } else {
